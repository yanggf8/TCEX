...
| WebSocket 即時推送 | ⚠️ | Client Store 已實作，後端連線部分尚為 Stub |
...
| 2 | WebSocket 後端連線 | P1 | Client Store 已就緒，後端 DO 連線尚未完整串接 |
| 3 | 掛牌管理後台 | P1 | 目前需手動執行 SQL，待建立 `/admin/listings` 介面 |
| 4 | KYC 文件預覽 | P2 | 目前須至 Cloudflare R2 後台查看，待加入 presigned URL |
| 5 | 入金自動對帳 | P2 | 待接入銀行 API（玉山/台新企業網銀） |
| 6 | LINE 推播通知 | P2 | Email 通知已上線，LINE Notify 尚未整合 |
| 7 | 洗錢防制監控 | P2 | 規則設計完成，自動偵測引擎待開發 |
| 8 | 金管會格式報表 | P2 | 待金管會監理沙盒申請確認格式需求 |
