...
**整體覆蓋率**：95%（程式碼完整，手機 OTP / LINE Login 需設定外部服務憑證）
...
| LINE 登入 | ⚠️ | 程式碼完整，但 `LINE_CHANNEL_ID/SECRET/REDIRECT_URI` 環境變數未設定，點擊回傳 503 |
| LINE 綁定/解綁 | ⚠️ | 同上，需先設定 LINE Login Channel 憑證才可用 |
...
| 2 | LINE Login 憑證設定 | P1 | 程式碼完整，需在 Cloudflare 設定 `LINE_CHANNEL_ID/SECRET/REDIRECT_URI` |
| 3 | 市價單前端參數化 | P2 | 引擎已支援，前後端硬寫為限價單，需開放 type 欄位 |
| 4 | 入金自動對帳 | P2 | 目前為管理員模擬入帳，待接入銀行 API（玉山/台新） |
