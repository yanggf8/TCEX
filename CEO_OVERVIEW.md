...
| 模組 | 用途 | 白話說明 |
|------|------|---------|
| 公開網站 + API | 展示平台資訊、處理資料請求、驗證身份 | 官方網站兼保全系統，同一套程式同時處理 |
| 交易核心 | 撮合買賣、即時推送、管理訂單、控管風險 | 平台的心臟，處理所有交易的地方 |
| 資料儲存 | 資料庫、快取、檔案、事件紀錄 | 平台的記憶體，安全保存所有紀錄 |

> **為什麼選擇 Cloudflare？** 不需要自己管理伺服器，費用極低（MVP 階段幾乎免費），全球部署速度快，台灣用戶體驗佳。若未來金管會要求資料必須存放在台灣境內機房，可無縫遷移至 GCP 台灣機房，不需要改寫程式碼。
...
| 2.3 | JWT 登入驗證 + 會員區域保護 | 安全的登入機制 |
| 2.4 | LINE 登入整合 | 台灣用戶一鍵登入 |
| 2.5 | 雙重驗證（2FA） | 交易安全保障 |
| 2.6 | KYC 身份驗證流程 | 從 Email 驗證到身分證上傳 |
| 2.7 | 會員專區基本架構 | 登入後的個人頁面 |

### 第三階段：交易核心

| 步驟 | 內容 | 交付成果 |
|------|------|---------|
| 3.1 | 完整資料庫架構（產品、訂單、交易、錢包） | 金融級資料儲存 |
| 3.2 | 掛牌產品 API | 產品目錄系統 |
| 3.3 | 錢包功能（入金、出金、餘額） | 資金管理 |
| 3.4 | 訂單管理（下單、取消、歷史） | 交易流程 |
| 3.5 | 撮合引擎原型（Durable Object） | 買賣自動配對 |
| 3.6 | 即時數據推送（WebSocket） | 即時報價與成交資訊 |
