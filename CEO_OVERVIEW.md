...
| 3.6 | WebSocket 即時推送（DO Hibernation API） ✅ | 即時掛單簿 + 成交推播、自動重連；portal 代理路由完整串接 |
...
| 4.5 | WebSocket 即時報價串接 ✅ | Portal 代理路由 → ENGINE 服務綁定 → Durable Object；頁面載入即顯示掛單簿，成交即時廣播 |
| 4.6 | KYC 文件預覽 ✅ | 管理員審核 L2 申請時，可直接在後台點擊查看身分證/文件圖片（R2 proxy，限 admin 存取） |
| 4.7 | 掛牌管理後台 ✅ | `/admin/listings`：列出所有掛牌、建立新掛牌（填表單）、一鍵暫停/恢復交易，不再需要手動 SQL |
| 4.8 | 模擬入帳 ✅ | 後台用戶管理頁 DEPOSIT 按鈕，管理員直接輸入金額入帳（MVP 無真實金流，完整稽核紀錄） |
| 4.9 | 稽核軌跡（不可竄改的紀錄） | 合規所需的交易紀錄 |
| 4.10 | 洗錢防制交易監控 | 可疑交易自動偵測 |
| 4.11 | 金管會格式報表 | 定期合規申報 |
| 4.12 | 金管會監理沙盒申請 | **啟動正式核准流程** |
...
*文件最後更新：民國115年2月（第二階段完整版 + 第三階段完成 + 安全強化 + Google OAuth + Resend Email + 後台管理系統 + 收入分成引擎 + 通知系統 + 首頁與儀表板即時數據串接 + 營運指引 + WebSocket 即時報價串接 + KYC 文件預覽 + 掛牌管理後台 + 模擬入帳）*
