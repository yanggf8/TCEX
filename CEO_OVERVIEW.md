...
| 功能 | 說明 |
|------|------|
| 後台管理系統 | 管理掛牌產品、審核 KYC、監控市場 |
| 市場控制 | 緊急暫停交易、熔斷機制管理 |
| 合規報表 | 自動產生金管會要求的各類報表 |
| 用戶管理 | 查看用戶資料、處理客訴、管理權限 |
...
| 模組 | 用途 | 白話說明 |
|------|------|---------|
| 公開網站 | 展示平台資訊、產品介紹 | 就像公司的官方網站，任何人都可以瀏覽 |
| API 閘道器 | 處理所有資料請求、驗證身份 | 像是大樓的保全系統，確認身份才放行 |
| 交易核心 | 撮合買賣、管理訂單、控管風險 | 平台的心臟，處理所有交易的地方 |
| 資料層 | 儲存所有資料、提供搜尋 | 平台的記憶體，安全保存所有紀錄 |
...
| 步驟 | 內容 | 交付成果 |
|------|------|---------|
| 2.1 | 建立後端 API 伺服器 | 後端基礎架構（GCP 台灣機房） |
| 2.2 | 註冊與登入功能 | 基本帳號系統 |
| 2.3 | LINE 登入整合 | 台灣用戶一鍵登入 |
| 2.4 | 雙重驗證（2FA） | 交易安全保障 |
| 2.5 | KYC 身份驗證流程 | 從 Email 驗證到身分證上傳 |
| 2.6 | 會員專區基本架構 | 登入後的個人頁面 |

### 第三階段：交易核心

| 步驟 | 內容 | 交付成果 |
|------|------|---------|
| 3.1 | 建立資料庫（GCP 台灣） | 金融級資料儲存 |
| 3.2 | 掛牌產品 API | 產品目錄系統 |
| 3.3 | 錢包功能（入金、出金、餘額） | 資金管理 |
| 3.4 | 訂單管理（下單、取消、歷史） | 交易流程 |
| 3.5 | 撮合引擎原型 | 買賣自動配對 |
| 3.6 | 即時數據推送 | 即時報價與成交資訊 |
| 3.7 | 交易介面（圖表、委託單、掛單簿） | 完整交易頁面 |

### 第四階段：收入分成 + 合規

| 步驟 | 內容 | 交付成果 |
|------|------|---------|
| 4.1 | 收入分成自動計算與發放 | 收入分成引擎 |
| 4.2 | 稽核軌跡（不可竄改的紀錄） | 合規所需的交易紀錄 |
| 4.3 | 洗錢防制交易監控 | 可疑交易自動偵測 |
| 4.4 | 金管會格式報表 | 定期合規申報 |
| 4.5 | 金管會監理沙盒申請 | **啟動正式核准流程** |

### 技術選型摘要

| 項目 | 選擇 | 原因 |
|------|------|------|
| 公開網站 | SvelteKit | 現代化網站框架，程式碼精簡、速度最快、SEO 佳 |
| 網站部署 | Cloudflare Pages | 全球邊緣部署，台灣用戶存取速度極快 |
| 交易引擎 | Rust 或 Go | 高效能，每秒可處理上萬筆訂單 |
| 資料庫 | PostgreSQL | 銀行級交易安全性 |
| 快取 | Redis | 即時數據推送 |
| 金融資料主機 | GCP 台灣機房 | 符合金管會資料在地化要求（必須在台灣） |
| 安全防護 | Cloudflare WAF | 防止網路攻擊、DDoS 防護 |
