...
<div class="min-h-[60vh] flex items-center justify-center px-4">
	<div class="w-full max-w-md">
		...
		<div class="bg-primary-50 border border-primary-200 rounded-lg p-8 text-center">
			...
			<h2 class="text-lg font-semibold text-gray-900 mb-2">{$t('register.comingSoon')}</h2>
			<p class="text-gray-600 text-sm mb-6">{$t('register.notice')}</p>

			{#if submitted}
				<div class="bg-green-50 border border-green-200 rounded-lg p-4">
					<p class="text-green-700 text-sm">{$t('register.notifySuccess')}</p>
				</div>
			{:else}
				<div class="text-left">
					<p class="text-sm text-gray-500 mb-3">{$t('register.notify')}</p>
		...
		<p class="text-center text-sm text-gray-500 mt-6">
			{$t('register.hasAccount')}<a href="/login" class="text-primary-600 hover:text-primary-700 ml-1">{$t('register.login')}</a>
		</p>
	</div>
</div>
